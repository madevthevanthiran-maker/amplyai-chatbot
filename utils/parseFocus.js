...
//   block 2025-09-10 09:00-11:00 for Something
// Times: "9", "9am", "9:30", "9:30pm", "21:00", "9-11", "9:00-11:15", "1pm-2pm"

function parseTimePiece(piece) {
  const s = piece.trim().toLowerCase();
  // 24h HH:MM or H:MM or HH
...
export function parseFocusText(input, defaultTz) {
  ...
  if (startMin == null || endMin == null) return { ok: false, error: "Could not parse the start/end time" };
  ...
  // Keep local date/time (no TZ conversion): "YYYY-MM-DDTHH:mm:ss"
  ...
  return {
    ok: true,
    data: { title, startISO: isoLocal(startDate), endISO: isoLocal(endDate), timezone: tz },
  };
}
