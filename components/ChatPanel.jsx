...
export default function ChatPanel({ mode, messages, onSend }) {
  ...
  useEffect(() => {
    ...
    taRef.current.style.height =
      Math.min(taRef.current.scrollHeight, 240) + "px";
  }, [input]);
  ...
  const submit = async () => {
    ...
    if (focus.handled) {
      ...
      if (focus.ok) setInput(""); // clear if event created
      try {
  ...
  return (
    <div className="w-full">
      ...
      <div className="h-[64vh] overflow-y-auto rounded-xl border border-slate-800 bg-slate-900/40 p-4">
        {messages.length === 0 ? (
          <div className="text-slate-400 text-sm">
            Ask anything. I can give structured answers with sources when
            useful. <br />
            Tip: type{" "}
            <code className="rounded bg-slate-800 px-1">block 2-4pm today for
            Deep Work</code>{" "}
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      code: ({ inline, className, children, ...props }) =>
                        inline ? (
                          <code className="rounded bg-slate-900/60 px-1 py-0.5">
                            {children}
                          </code>
                        ) : (
      ...
      <div className="mt-3 flex flex-col gap-2">
        {ephemeralNotice && (
          <div className="text-sm text-slate-300">{ephemeralNotice}</div>
        )}
